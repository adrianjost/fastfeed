...
function save_feed_item($data){
    ...
    global $db_prefix;
    $sql = "INSERT INTO ".$db_prefix."FEEDDATA (`cdate`, `feedid`, `url`, `title`, `preview`) VALUES (".time().", ".$data["feedid"].", '".substr(trim($data["url"]),0,2000)."', '".utf8_encode(substr(trim($data["title"]),0,200))."', '".utf8_encode((trim($data["preview"])))."');";
    db_query($sql);
}
