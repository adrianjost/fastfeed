...
/* --------------------------------------------------------
	CARDS
---------------------------------------------------------*/
function get_card(id,content){return '<a class="card-panel" id="'+id+'" rel="nofollow" href="#'+id+'" url="'+content["url"]+'"><div class="headline"><img class="favicon" role="presentation" src="'+content["iconurl"]+'"></img><h2 class="title">'+content["title"]+'</h2></div><div class="content"><p>'+content["preview"]+'</p></div></a>'}
function get_fullcard(content,a){return '<div class="card-panel"><div id="singleheadline" class="headline"><img class="favicon" role="presentation" src="'+content["iconurl"]+'"></img><h2 class="title">'+content["title"]+'</h2></div><div class="content">'+a+'</div>'+get_share_links(content["url"])+'</div>'}
...
function loadarticle(t){
	...
	if(c["fullcontent"]){
		...
		ajax(process.env.API_URL + "/getarticle.php?id="+t.getAttribute("id")+"&url="+encodeURIComponent(t.getAttribute("url")),function(rr){
			if(!rr){
				t.classList.remove('loading');
...
window.addEventListener("load", () => {
	...
	ajax(process.env.API_URL + "/getfeedupdates.php?last="+((last==-Infinity)?0:last)+"&uid="+userid,function(resp){
